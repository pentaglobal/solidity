...
void VarDeclPropagator::operator()(Assignment& _assignment)
{
	...
	if (checkAllVarNamesUninitialized(_assignment.variableNames))
...
bool VarDeclPropagator::checkAllVarNamesUninitialized(vector<Identifier> const& _variableNames)
{
	return all_of(
		begin(_variableNames),
		end(_variableNames),
		[&](Identifier const& _ident) { return m_emptyVarDecls.count(_ident.name); }
	);
}
...
bool VarDeclPropagator::isFullyLazyInitialized(vector<Identifier> const& _variableNames) const
{
	return all_of(
		...
		[&](Identifier const& ident) { return m_lazyInitializedVarDecls.count(ident.name); }
	);
}
...
VariableDeclaration VarDeclPropagator::recreateVariableDeclaration(Assignment& _assignment)
{
	...
	{
		aux.emplace_back(move(m_lazyInitializedVarDecls.at(varName.name)));
		m_lazyInitializedVarDecls.erase(varName.name);
	}

	return VariableDeclaration{move(_assignment.location), move(aux), std::move(_assignment.value)};
}
